...
// Update DevicePriceByDeviceZoneId
export const updateDevicePriceByDeviceZoneId =
  (deviceId, zoneId, devicePriceData) => async (dispatch) => {
    try {
      dispatch({ type: UPDATE_DEVICE_PRICE_BY_DEVICE_ZONE_ID_REQUEST });
      ...
      const { data } = await axiosInstance.put(
        `/api/v1/devicePriceByDeviceZone/${deviceId}/${zoneId}`,
        devicePriceData,
        config
      );

      dispatch({
        type: UPDATE_DEVICE_PRICE_BY_DEVICE_ZONE_ID_SUCCESS,
        payload: data.success,
      });
    } catch (error) {
      dispatch({
        type: UPDATE_DEVICE_PRICE_BY_DEVICE_ZONE_ID_FAIL,
        payload: error.response.data.message,
      });
    }
  };
...
// Get DevicePricesByDeviceZoneId Details
export const getDevicePriceDetailsByDeviceZoneId =
  (deviceId, zoneId) => async (dispatch) => {
    try {
      dispatch({ type: DEVICE_PRICE_DETAILS_BY_DEVICE_ZONE_ID_REQUEST });

      const { data } = await axiosInstance.get(
        `/api/v1/devicePriceByDeviceZone/${deviceId}/${zoneId}`
      );
      dispatch({
        type: DEVICE_PRICE_DETAILS_BY_DEVICE_ZONE_ID_SUCCESS,
        payload: data.devicePrice,
      });
    } catch (error) {
      dispatch({
        type: DEVICE_PRICE_DETAILS_BY_DEVICE_ZONE_ID_FAIL,
        payload: error.response.data.message,
      });
    }
  };
