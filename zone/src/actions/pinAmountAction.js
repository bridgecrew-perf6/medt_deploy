...
export const getAdminPinAmount = () => async (dispatch) => {
  try {
    ...
    const { data } = await axiosInstance.get("/api/v1/pinAmounts");
...
export const deletePinAmount = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axiosInstance.delete(`/api/v1/pinAmount/${id}`);
...
export const getPinAmountDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axiosInstance.get(`/api/v1/pinAmount/${id}`);
