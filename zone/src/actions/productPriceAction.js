...
import { PRODUCT_PRICE_DETAILS_BY_PRODUCT_ZONE_ID_REQUEST } from "../constants/productPriceConstants";
import { PRODUCT_PRICE_DETAILS_BY_PRODUCT_ZONE_ID_SUCCESS } from "../constants/productPriceConstants";
...
// Update ProductPriceByProductZoneId
export const updateProductPriceByProductZoneId =
  (productId, zoneId, productPriceData) => async (dispatch) => {
    try {
      dispatch({ type: UPDATE_PRODUCT_PRICE_BY_PRODUCT_ZONE_ID_REQUEST });
      ...
      const { data } = await axiosInstance.put(
        `/api/v1/productPriceByProductZone/${productId}/${zoneId}`,
        productPriceData,
        config
      );

      dispatch({
        type: UPDATE_PRODUCT_PRICE_BY_PRODUCT_ZONE_ID_SUCCESS,
        payload: data.success,
      });
    } catch (error) {
      dispatch({
        type: UPDATE_PRODUCT_PRICE_BY_PRODUCT_ZONE_ID_FAIL,
        payload: error.response.data.message,
      });
    }
  };
...
// Get ProductPricesByProductZoneId Details
export const getProductPriceDetailsByProductZoneId =
  (productId, zoneId) => async (dispatch) => {
    try {
      dispatch({ type: PRODUCT_PRICE_DETAILS_BY_PRODUCT_ZONE_ID_REQUEST });

      const { data } = await axiosInstance.get(
        `/api/v1/productPriceByProductZone/${productId}/${zoneId}`
      );
      dispatch({
        type: PRODUCT_PRICE_DETAILS_BY_PRODUCT_ZONE_ID_SUCCESS,
        payload: data.productPrice,
      });
    } catch (error) {
      dispatch({
        type: PRODUCT_PRICE_DETAILS_BY_PRODUCT_ZONE_ID_FAIL,
        payload: error.response.data.message,
      });
    }
  };
